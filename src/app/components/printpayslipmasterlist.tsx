...
const PrintPayslipMasterList = () => {
  ...
  return (
    <div className="fixed bottom-1 right-50 w-60 bg-white border border-gray-300 shadow-md">
      ...
      {isboxVisible && (
        <>
          <div className="flex-grow overflow-y-auto p-3 mx-auto w-[800px] font-bold">
            <table>
                {Array.from(new Set(sched.map((list) => list.pym))).map((unique) => (
                    <div key={unique}>
                        <Link href={`/masterlistpayslip/${unique}`}>{unique}</Link>
                    </div>
                ))}
            </table>
          </div>
      ...
      <div className="flex justify-center p-2 bg-gray-600 w-[400px] rounded-b-md">
...
export default PrintPayslipMasterList;
