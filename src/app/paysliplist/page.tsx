...
const Payroll=()=>{
    ...
    useEffect(() => {
      ...
      const handleDeleteTask = async (list) => {
        ...
        if (isConfirmed) {
          try {
            const response = await fetch(`http://localhost:7000/payslip/${list.id}`, {
              method: 'DELETE',
              headers: {
                'Content-Type': 'application/json',
              },
    ...
    return (
        <>
            <div className="grid grid-cols-6">
                ...
                <div className="text-xl font-bold">Finalized Payslip</div>
                <div></div>
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-900">
                    <thead className="text-xs text-gray-900 uppercase bg-gray-100 dark:bg-gray-600 dark:text-gray-100">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3">NetPay</th>
                            <th scope="col" className="px-6 py-3 text-center"></th>
                            <th scope="col" className="px-6 py-3">View Payslip</th>
